...
def onefile(fnm=None, ext=None, err=False):
    ...
    if fnm is not None:
        if os.path.exists(fnm):
            if os.path.dirname(os.path.abspath(fnm)) != os.getcwd():
                ...
                if os.path.exists(fdest):
                    if not filecmp.cmp(fsrc, fdest):
                        logger.error(
                            f"onefile() will not overwrite {os.path.join(os.getcwd(), os.path.basename(fnm))} with {os.path.abspath(fnm)}\n"
                        )
                        raise RuntimeError
                    else:
                        logger.info(
                            f"\x1b[93monefile() says the files {os.path.abspath(fnm)} and {os.getcwd()} are identical\x1b[0m\n"
                        )
                else:
                    logger.info(
                        f"\x1b[93monefile() will copy {os.path.abspath(fnm)} to {os.getcwd()}\x1b[0m\n"
                    )
            ...
            warn_once(
                f"File specified by {fnm} does not exist - will try to autodetect .{ext} extension"
            )
    answer = None
    cwd = os.getcwd()
    ...
    if len(ls) != 1:
        ...
        warn_once(
            f"Autodetected {answer} in {cwd}", warnhash="Autodetected %s" % answer
        )
    return answer
...
def listfiles(fnms=None, ext=None, err=False, dnm=None):
    ...
    if answer == [] and err:
        logger.error(
            f"listfiles function failed to come up with a file! (fnms = {str(fnms)} ext = {str(ext)})"
        )
        raise RuntimeError
...
def Leave(Dir):
    if os.path.split(os.getcwd())[1] != Dir:
        logger.error(
            f"Trying to leave directory {Dir}, but we're actually in directory {os.path.split(os.getcwd())[1]} (check your code)\n"
        )
        raise RuntimeError
    ...
    if os.path.exists(src):
        ...
        if os.path.islink(dest) and not os.path.exists(dest):
            ...
            if os.path.islink(dest):
                ...
                logger.error(
                    f"Tried to create symbolic link {src} to {dest}, destination exists but isn't a symbolic link\n"
                )
                raise RuntimeError
        else:
            os.symlink(src, dest)
        ...
        if not nosrcok:
            logger.error(
                f"Tried to create symbolic link {src} to {dest}, but source file doesn't exist{MissingFileInspection(src)}\n"
            )
            raise RuntimeError
...
def CopyFile(src, dest):
    if os.path.exists(src):
        if os.path.exists(dest):
            if os.path.islink(dest):
                logger.error(
                    f"Tried to copy {src} to {dest}, destination exists but it's a symbolic link\n"
                )
                raise RuntimeError
        else:
        ...
        logger.error(
            f"Tried to copy {src} to {dest}, but source file doesn't exist{MissingFileInspection(src)}\n"
        )
        raise RuntimeError
