        ...
        for line in open(fin).readlines():
            ...
            if key in options:
                ...
                if key in clashes and val != val0:
                    logger.error(
                        f"edit_mdp tried to set {key} = {val} but its original value was {key} = {val0}\n"
                    )
                    raise RuntimeError
...
class GMX(Engine):
    ...
    def prepare(self, pbc=False, **kwargs):
        ...
        if pbc:
            ...
            if rlist > 0.05 * (float(int(minbox - 1))):
                warn_press_key(
                    f"nonbonded_cutoff = {rlist*10:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                )
            ...
            if rvdw > 0.05 * (float(int(minbox - 1))):
                warn_press_key(
                    f"vdw_cutoff = {rvdw*10:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                )
        ...
        o = self.warngmx(
            f"grompp -c {self.name}.gro -p {self.name}.top -f {self.name}.mdp -o {self.name}.tpr",
            warnings=warnings,
        )
        self.double = 0
        for line in o:
    ...
    def optimize(self, shot, crit=1e-4, align=True, **kwargs):
        ...
        self.warngmx(
            f"grompp -c {self.name}.gro -p {self.name}.top -f {self.name}-min.mdp -o {self.name}-min.tpr"
        )
        self.callgmx("mdrun -deffnm %s-min -nt 1" % self.name)
        self.callgmx(
            f"trjconv -f {self.name}-min.trr -s {self.name}-min.tpr -o {self.name}-min.gro -ndec 9",
            stdin="System",
        )
        self.callgmx(
            f"g_energy -xvg no -f {self.name}-min.edr -o {self.name}-min-e.xvg",
            stdin="Potential",
        )
    ...
    def evaluate_(self, force=False, dipole=False, traj=None):
        ...
        self.warngmx(
            f"grompp -c {self.name}.gro -p {self.name}.top -f {self.name}-1.mdp -o {self.name}.tpr"
        )
        self.callgmx(
        ...
        if force:
            self.callgmx(
                f"g_traj -xvg no -s {self.name}.tpr -f {self.name}.trr -of {self.name}-f.xvg -fp",
                stdin="System",
            )
    ...
    def make_gro_trajectory(self, fout=None):
        ...
        self.callgmx(
            f"trjconv -f {self.mdtraj} -o {fout} -ndec 9 -pbc mol -novel -noforce",
            stdin="System",
        )
        return fout
    ...
    def energy_rmsd(self, shot, optimize=True):
        ...
        if optimize:
            ...
            self.mol[shot].write("%s.gro" % self.name)
            self.warngmx(
                f"grompp -c {self.name}.gro -p {self.name}.top -f {self.name}.mdp -o {self.name}-1.tpr"
            )
            self.callgmx(f"mdrun -deffnm {self.name}-1 -nt 1 -rerunvsite")
            self.callgmx(
                f"g_energy -xvg no -f {self.name}-1.edr -o {self.name}-1-e.xvg",
                stdin="Potential",
            )
    ...
    def interaction_energy(self, fraga, fragb):
        ...
        self.callgmx(
            f"mdrun -deffnm {self.name}-i -nt 1 -rerunvsite -rerun {self.name}-all.gro"
        )
        self.callgmx(
        ...
        self.callgmx(
            f"mdrun -deffnm {self.name}-x -nt 1 -rerunvsite -rerun {self.name}-all.gro"
        )
        self.callgmx(
    ...
    def normal_modes(self, shot=0, optimize=True):
        ...
        if optimize:
            self.optimize(shot)
            self.warngmx(
                f"grompp -c {self.name}-min.gro -p {self.name}.top -f {self.name}-nm.mdp -o {self.name}-nm.tpr"
            )
        else:
            warn_once("Asking for normal modes without geometry optimization?")
            self.mol[shot].write("%s.gro" % self.name)
            self.warngmx(
                f"grompp -c {self.name}.gro -p {self.name}.top -f {self.name}-nm.mdp -o {self.name}-nm.tpr"
            )
        ...
        self.callgmx(
            f"trjconv -s {self.name}-nm.tpr -f {self.name}-nm.trr -o {self.name}-nm.gro -ndec 9",
            stdin="System",
        )
    ...
    def generate_positions(self):
        ...
        self.callgmx(
            f"mdrun -deffnm {self.name} -nt 1 -rerunvsite -rerun {self.name}-all.gro"
        )
        self.callgmx(
            f"trjconv -f {self.name}.trr -o {self.name}-out.gro -ndec 9 -novel -noforce",
            stdin="System",
        )
    ...
    def molecular_dynamics(
        ...
        **kwargs,
        ...
        if nequil > 0:
            ...
            self.warngmx(
                f"grompp -c {gro1} -p {self.name}.top -f {self.name}-eq.mdp -o {self.name}-eq.tpr",
                warnings=warnings,
        ...
        self.warngmx(
            f"grompp -c {gro2} -p {self.name}.top -f {self.name}-md.mdp -o {self.name}-md.tpr",
            warnings=warnings,
            print_command=verbose,
        )
        self.callgmx(
        ...
        self.callgmx(
            f"g_dipoles -s {self.name}-md.tpr -f {self.name}-md.trr -o {self.name}-md-dip.xvg -xvg no",
            stdin="System\n",
        )
        ...
        if bilayer:
            ...
            al_vars = ["Box-Y", "Box-X"]
            self.callgmx(
                f"g_energy -f {self.name}-md.edr -o {self.name}-md-energy-xy.xvg -xvg no",
                stdin="\n".join(al_vars),
            )
            Xs = []
            Ys = []
        ...
        self.callgmx(
            f"g_energy -f {self.name}-md.edr -o {self.name}-md-energy.xvg -xvg no",
            stdin="\n".join(ekeep),
        )
        ecomp = OrderedDict()
        Rhos = []
...
class Liquid_GMX(Liquid):
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if (temperature, pressure) in self.LfDict:
            ...
            logger.info(
                f"Copying previous iteration final geometry .gro file: {lfsrc} to {lfdest}\n"
            )
