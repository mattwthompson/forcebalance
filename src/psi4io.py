...
from forcebalance.leastsq import CheckBasis, LeastSquares
    ...
    def indicate(self):
        ...
        logger.info(
            ...
            % (
                self.name,
                str(self.Molecules),
                f"Mean (Max) Error: {100*MAD / self.MAQ:8.4f}% ({100*np.max(np.abs(self.D)) / self.MAQ:8.4f}%) Energies: DF {self.DF_Energy:+.3e} MP2 {self.MP2_Energy:+.3e} Delta {self.DF_Energy - self.MP2_Energy: .3e} Objective = {self.objective:.5e}",
            )
        )
        return
