...
try:
    from forcebalance.psi4io import RDVR3_Psi4, THCDF_Psi4
except:
    logger.warning(traceback.format_exc())
...
class Penalty:
    ...
    def __init__(
        ...
        if User_Option.upper() == "L1":
            ...
            logger.info(
                f"Using hyperbolic regularization (Laplacian prior) with strength {Factor_Add:.1e} (+), {Factor_Mult:.1e} (x) and tightness {Factor_B:.1e}\n"
            )
        elif self.ptyp == 2:
            if Power == 2.0:
                logger.info(
                    f"Using parabolic regularization (Gaussian prior) with strength {Factor_Add:.1e} (+), {Factor_Mult:.1e} (x)\n"
                )
            elif Power > 2.0:
                logger.info(
                    f"Using customized L2-regularization with exponent {Power:.1f}, strength {Factor_Add:.1e} (+), {Factor_Mult:.1e} (x)\n"
                )
            else:
                logger.error(
            ...
            if Power == 2.0:
                logger.info(
                    f"Using box-style regularization with exponent {Power:.1f}, strength {Factor_Add:.1e} (+), {Factor_Mult:.1e} (x): same as L2\n"
                )
            elif Power > 2.0:
                logger.info(
                    f"Using box-style regularization with exponent {Power:.1f}, strength {Factor_Add:.1e} (+), {Factor_Mult:.1e} (x)\n"
                )
            else:
                logger.error(
            ...
            logger.info(
                f"Using L0-L1 Fusion Penalty (only relevant for basis set optimizations at the moment) with strength {Factor_Add:.1e} and switching distance {Alpha:.1e}\n"
            )
        elif self.ptyp == 6:
            logger.info(
                f"Using L1 Fusion Penalty with Log Barrier (only relevant for basis set optimizations at the moment) with strength {Factor_Add:.1e} and barrier distance {Alpha:.1e}\n"
            )
