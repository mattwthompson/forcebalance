...
from forcebalance.nifty import logger, np, os, printcool, which
...
class TestAmber99SB(ForceBalanceTestCase):
    ...
    def setup_class(cls):
        ...
        if tinkerpath != "":
            cls.engines["TINKER"] = TINKER(
                coords="all.arc", tinker_key="alaglu.key", tinkerpath=tinkerpath
            )
        else:
...
class TestAmoebaWater6(ForceBalanceTestCase):
    ...
    def setup_class(cls):
        ...
        if tinkerpath:
            cls.T = TINKER(
                coords="hex.arc", tinker_key="prism.key", tinkerpath=tinkerpath
            )
    ...
    def test_energy_force(self):
        ...
        np.testing.assert_allclose(
            EF_O[0],
            EF_R[0],
            err_msg="OpenMM energy does not match the reference",
            rtol=0,
            atol=0.001,
        )
        np.testing.assert_allclose(
            EF_T[0],
            EF_R[0],
            err_msg="TINKER energy does not match the reference",
            rtol=0,
            atol=0.001,
        )
        ...
        np.testing.assert_allclose(
            EF_O[1:],
            EF_R[1:],
            err_msg="OpenMM forces do not match the reference",
            rtol=0,
            atol=0.01,
        )
        np.testing.assert_allclose(
            EF_T[1:],
            EF_R[1:],
            err_msg="TINKER forces do not match the reference",
            rtol=0,
            atol=0.01,
        )
    ...
    def test_energy_rmsd(self):
        ...
        np.testing.assert_allclose(
            EO,
            ERef,
            err_msg="OpenMM minimized energy does not match the reference",
            rtol=0,
            atol=0.0001,
        )
        np.testing.assert_allclose(
            ET,
            ERef,
            err_msg="TINKER minimized energy does not match the reference",
            rtol=0,
            atol=0.0001,
        )
        ...
        np.testing.assert_allclose(
            RO,
            RRef,
            err_msg="OpenMM RMSD does not match the reference",
            rtol=0,
            atol=0.001,
        )
        np.testing.assert_allclose(
            RT,
            RRef,
            err_msg="TINKER RMSD does not match the reference",
            rtol=0,
            atol=0.001,
        )
    ...
    def test_interaction_energy(self):
        ...
        np.testing.assert_allclose(
            IO,
            IR,
            err_msg="OpenMM interaction energies do not match the reference",
            rtol=0,
            atol=0.0001,
        )
        np.testing.assert_allclose(
            IT,
            IR,
            err_msg="TINKER interaction energies do not match the reference",
            rtol=0,
            atol=0.0001,
        )
    ...
    def test_multipole_moments(self):
        ...
        np.testing.assert_allclose(
            DO,
            DR,
            err_msg="OpenMM dipoles do not match the reference",
            rtol=0,
            atol=0.001,
        )
        np.testing.assert_allclose(
            DT,
            DR,
            err_msg="TINKER dipoles do not match the reference",
            rtol=0,
            atol=0.001,
        )
        ...
        np.testing.assert_allclose(
            QO,
            QR,
            err_msg="OpenMM quadrupoles do not match the reference",
            rtol=0,
            atol=0.001,
        )
        np.testing.assert_allclose(
            QT,
            QR,
            err_msg="TINKER quadrupoles do not match the reference",
            rtol=0,
            atol=0.001,
        )
