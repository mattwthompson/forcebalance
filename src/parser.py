...
for i in all_opts_names:
    ...
    if len(iocc) != 1:
        logger.error(
            f"CODING ERROR: ForceBalance option {i} occurs in more than one place ({str(iocc)})\n"
        )
        raise RuntimeError
            ...
            if re.match(r"^\$", line):
                ...
                if len(s) > 1 and s[1].upper() == "NONE":
                    ...
                    if len(s) == 1:
                        ...
                        logger.error(
                            f'{key} is a true/false option but you provided {s[1]}; to enable, provide ["1", "yes", "true", "on" or <no value>].  To disable, provide ["0", "no", "false", or "off"].\n'
                        )
                        raise RuntimeError
