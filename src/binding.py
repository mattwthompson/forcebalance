...
from forcebalance.finite_difference import f1d2p, f12d3p, fdwrap, in_fd
    ...
    for ln, line in enumerate(fobj):
        ...
        if re.match(r"^\$", line):
            ...
            if word == "END":  # End of a section, time to reinitialize variables.
                ...
                warn_press_key(
                    f"Encountered section keyword {word} when already in section {section}"
                )
        elif section == "GLOBAL":
...
class BindingEnergy(Target):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        if self.attenuate:
            ...
            logger.info(
                f"Interaction energies more positive than {denom:.1f} will have reduced weight going as: 1.0 / ({denom:.1f}^2 + (energy-{denom:.1f})^2)\n"
            )
        ...
        def compute(mvals_):
            ...
            for sys_ in self.sys_opts:
                ...
                if not in_fd() and RMSD_ != 0.0:
                    self.RMSDDict[sys_] = f"{RMSD_: 9.3f} {w_*RMSDNrm_**2: 12.5f}"
            VectorE_ = []
            for inter_ in self.inter_opts:
                ...
                if not in_fd():
                    self.PrintDict[
                        inter_
                    ] = f"{Calculated_: 9.3f} {Reference_: 9.3f} {Delta_: 9.3f} {w_*DeltaNrm_**2: 12.5f}"
