...
try:
    ...
    package_install_dir = os.path.split(
        importlib.util.find_spec(__name__.split(".")[0]).origin
    )[0]
...
if "forcebalance" in __name__:
    ...
    for fnm in [
        ...
        os.path.join(
            package_install_dir, "_dcdlib" + str(sysconfig.get_config_var("EXT_SUFFIX"))
        ),
...
def extract_int(arr, avgthre, limthre, label="value", verbose=True):
    ...
    if abs(maximum - minimum) > limthre:
        if verbose:
            logger.info(
                f"Maximum {label} fluctuation ({abs(maximum-minimum):f}) is larger than threshold of {limthre:f}"
            )
        passed = False
...
class Molecule:
        ...
        for key in self.Data:
            if key in [
                ...
                if not isinstance(self.Data[key], list):
                    raise RuntimeError(
                        f"Expected data attribute {key} to be a list, but it is {str(type(self.Data[key]))}"
                    )
    ...
    def replace_peratom_conditional(self, key1, cond, key2, orig, want):
        ...
        if key2 in self.Data and key1 in self.Data:
            ...
            logger.error(
                f"Either the comparison or replacement key ({key1}, {key2}) doesn't exist.\n"
            )
            raise RuntimeError
    ...
    def atom_select(self, atomslice, build_topology=True):
        ...
        for key in self.AtomKeys:
            if key == "tinkersuf":  # Tinker suffix is a bit tricky
                Map = {a + 1: i + 1 for i, a in enumerate(atomslice)}
        ...
        for key in self.AtomKeys:
            ...
            if key == "tinkersuf":  # Tinker suffix is a bit tricky
                ...
                if type(self.Data[key]) is np.ndarray:
                    ...
                    logger.error(
                        f"Cannot stack {key} because it is of type {str(type(New.Data[key]))}\n"
                    )
                    raise RuntimeError
    ...
    def rotate_check_clash(
        ...
        if not (haveClash_H or haveClash_C):
            if printLevel >= 1:
                print(
                    f"\n    \x1b[1;92mSuccess - no clashes. Thresh(H, Hvy) = ({thresh_hyd:.2f}, {thresh_hvy:.2f})\x1b[0m"
                )
        ...
        for line in open(fnm):
            ...
            if title:
                ...
                if (
                    len(sline) == 6
                    and isfloat(sline[1])
                    and all([isfloat(i) for i in sline])
                    ...
                    if nframes == 0:
                        ...
                        if len(sline) > 5:
                            ...
                            tinkersuf.append(
                                "".join(
                                    [whites[j] + s[j - 5] for j in range(5, len(sline))]
                                )
                            )
                        else:
                            tinkersuf.append("")
    ...
    def read_qcout(self, fnm, errok=None, **kwargs):
        ...
        if "qm_energies" in Answer:
            ...
            if len(set(lens)) != 1:
                logger.error(
                    f"The number of energies and coordinates in {fnm} are not the same : {str(lens)}\n"
                )
                raise RuntimeError
        ...
        for SI, I in enumerate(selection):
            ...
            if "qm_extchgs" in self.Data:
                ...
                for i in range(len(extchg)):
                    out.append(
                        f"{extchg[i,0]: 15.10f} {extchg[i,1]: 15.10f} {extchg[i,2]: 15.10f} {extchg[i,3]:15.10f}"
                    )
                out.append("$end")
    ...
    def write_molproq(self, selection, **kwargs):
        ...
        for I in selection:
            ...
            for i in range(self.na):
                out.append(
                    f"{xyz[i,0]: 15.10f} {xyz[i,1]: 15.10f} {xyz[i,2]: 15.10f} {self.partial_charge[i]: 15.10f}   0"
                )
        return out
    ...
    def write_arc(self, selection, **kwargs):
        ...
        for I in selection:
            ...
            if "boxes" in self.Data:
                b = self.boxes[I]
                out.append(
                    f" {b.a:11.6f} {b.b:11.6f} {b.c:11.6f} {b.alpha:11.6f} {b.beta:11.6f} {b.gamma:11.6f}"
                )
            for i in range(self.na):
                out.append(
                    "%6i  %s%s"
