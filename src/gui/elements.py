...
from forcebalance.output import INFO, StreamHandler, getLogger
...
class ObjectViewer(tk.LabelFrame):
    ...
    def __init__(self, root, **kwargs):
        ...
        self.content = tk.Text(
            self, cursor="arrow", state="disabled", width=30, height=20
        )
    ...
    def update(self, *args):
        ...
        if self.calculation:
            ...
            self.content.bind(
                "<Button-1>", _bindEventHandler(self.select, object=[self.calculation])
            )
            ...
            l.bind(
                "<Button-1>",
                _bindEventHandler(self.select, object=[self.calculation["options"]]),
            )
            self.content.insert("end", "\n")

            # Event handler to toggle whether targets list should be expanded
            def toggle(e):
                self.calculation["_expand_targets"] = not self.calculation[
                    "_expand_targets"
                ]
                self.needUpdate.get()
            ...
            targetLabel.bind(
                "<Button-1>",
                _bindEventHandler(self.select, object=self.calculation["targets"]),
            )
            ...
            if self.calculation["_expand_targets"]:
                ...
                for target in self.calculation["targets"]:
                    ...
                    self.content.insert("end", "\n")
                    l.bind(
                        "<Button-1>", _bindEventHandler(self.select, object=[target])
                    )
            else:
                self.content.insert("end", "+")
                self.content.window_create("end", window=targetLabel)
            ...
            l.bind(
                "<Button-1>",
                _bindEventHandler(self.select, object=[self.calculation["forcefield"]]),
            )
            self.content.insert("end", "\n")
            ...
            if self.calculation["result"]:
                ...
                l.bind(
                    "<Button-1>",
                    _bindEventHandler(self.select, object=[self.calculation["result"]]),
                )
                self.content.insert("end", "\n\n")
...
class DetailViewer(tk.LabelFrame):
    ...
    def __init__(self, root, opts="", **kwargs):
        ...
        self.scrollbar = tk.Scrollbar(self, orient=tk.VERTICAL)
        self.helptext = tk.Text(
            self.root, width=70, state="disabled", bg="#F0F0F0", wrap=tk.WORD
        )
    ...
    def load(self, newObject=None):
        ...
        if self.currentObject:
            if (
                len(self.currentObject) == 1
            ):  # if there is an object to display and it is not a collection
            ...
            try:
                ...
                self.content.insert(
                    ...
                    "Error trying to display <{} {}>\n".format(
                        self.currentObject[0]["type"], self.currentObject[0]["name"]
                    ),
                    "error",
                )
    ...
    def populate(self, object):
        ...
        if type(displayText) == dict:
            for key in displayText.keys():
                ...
                valuelabel = tk.Label(
                    frame, text=str(displayText[key]), bg="#FFFFFF", padx=0, pady=0
                )
        ...
        if type(displayText) == tuple:
            for key in displayText[0].keys():
                ...
                separator.bindtags((key, "scrollable"))
                valuelabel = tk.Label(
                    frame, text=str(displayText[0][key]), bg="#FFFFFF", padx=0, pady=0
                )
                valuelabel.bindtags((key, "scrollable"))
                ...
                self.root.bind_class(
                    key,
                    "<Button-3>",
                    _bindEventHandler(self.showHelp, object=object, option=key),
                )
            ...
            if self.printAll.get():
                ...
                for key in displayText[1].keys():
                    ...
                    keylabel.bindtags((key, "scrollable"))
                    separator = tk.Label(
                        frame, text=" : ", bg="#FFFFFF", padx=0, pady=0
                    )
                    separator.bindtags((key, "scrollable"))
                    valuelabel = tk.Label(
                        frame,
                        text=str(displayText[1][key]),
                        bg="#FFFFFF",
                        padx=0,
                        pady=0,
                    )
    ...
    def showHelp(self, e, object, option):
        ...
        self.helptext.place(
            x=e.x_root - self.root.winfo_x(), y=e.y_root - self.root.winfo_y()
        )
...
class ConsoleViewer(tk.LabelFrame):
    ...
    def __init__(self, root, **kwargs):
        ...
        self.console = tk.Text(
            self, state=tk.DISABLED, cursor="arrow", fg="#FFFFFF", bg="#000000"
        )
...
class ConsolePaneHandler(StreamHandler):
    ...
    def emit(self, record):
        ...
        message = re.split(
            "(\x1b\\[[01]?;?[0-9]{1,2};?[0-9]{,2}m)", record.getMessage()
        )
