...
class ForceBalanceObject:
    ...
    def display(self, verbose=False):
        ...
        for key in self.properties.keys():
            s += f"{key} : {str(self.properties[key])}\n"
        return s
...
class CalculationObject(ForceBalanceObject):
    ...
    def __init__(self, filename=None):
        super().__init__(type="calculation")
    ...
    def writeOptions(self, filename):
        with open("~" + filename, "w") as f:
            ...
            for option in self.properties["options"].opts.keys():
                f.write(
                    "{} {}\n".format(option, self.properties["options"].opts[option])
                )
            f.write("$end\n")
            ...
            for target in self.properties["targets"]:
                ...
                for option in target.opts:
                    f.write(f"{option} {target.opts[option]}\n")
                f.write("$end\n")
    ...
    def run(self):
        try:
            ...
            objective = forcebalance.objective.Objective(
                self.opts, self.tgt_opts, self.properties["forcefield"].forcefield
            )
            ## The optimizer component of the project
            optimizer = forcebalance.optimizer.Optimizer(
                self.opts, objective, self.properties["forcefield"].forcefield
            )
...
class OptionObject(ForceBalanceObject):
    ...
    def __init__(self, opts=None, name="unknown options file"):
        ...
        if not opts:
            self.opts = forcebalance.parser.gen_opts_defaults
        else:
            self.opts = opts
    ...
    def display(self, verbose=False):
        ...
        for key in self.opts.keys():
            if not self.isDefault(key):
                ...
                default_options[key] = self.opts[key]
    ...
    def isDefault(self, option):
        return (
            option in forcebalance.parser.gen_opts_defaults.keys()
            and self.opts[option] == forcebalance.parser.gen_opts_defaults[option]
        )
...
class TargetObject(ForceBalanceObject):
    ...
    def isDefault(self, option):
        return (
            option in forcebalance.parser.tgt_opts_defaults.keys()
            and self.opts[option] == forcebalance.parser.tgt_opts_defaults[option]
        )
...
class ForcefieldObject(ForceBalanceObject):
    ...
    def __init__(self, opts):
        super().__init__(type="forcefield")
        self.opts = opts
