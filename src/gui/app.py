...
class MainWindow(tk.Tk):
    ...
    def _initialize_menu(self):
        ...
        optionsmenu.add_checkbutton(
            label="show object viewer", variable=self.showObjects
        )
        optionsmenu.add_checkbutton(
            label="show default options", variable=self.detailsPane.printAll
        )
        self.menubar.add_cascade(label="Options", menu=optionsmenu)

        self.menubar.add_command(
            label="Run", command=self.objectsPane.run, state="disabled"
        )
    ...
    def open(self):
        ...
        inputfile = tkfile.askopenfilename(
            title="Open ForceBalance input file...", filetypes=filters
        )
        if inputfile:
