...
parser.add_argument(
    "engine",
    help='MD program that we are using; choose "openmm", "tinker" or "gromacs"',
)
...
def main():
    ...
    Sep = printcool(
        f"Average Area per Lipid: {Al_avg: .4f} +- {Al_err: .4f} nm^2\nAnalytic Derivative:"
    )
    ...
    Sep = printcool(
        f"Lipid Isothermal compressibility:  {LKappa: .4e} +- {LKappa_err:.4e} N/nm^-1\nAnalytic Derivative:"
    )
    ...
    if FDCheck:
        ...
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i-j: .4e}  {(i-j)/j: .4e}" for i, j in zip(GLKappa, GLKappa_fd)]
        FF.print_map(vals=absfrac)
