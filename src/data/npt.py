...
from forcebalance.finite_difference import f1d2p, f1d7p, f12d3p, fdwrap, in_fd
...
parser.add_argument(
    "engine",
    help='MD program that we are using; choose "openmm", "tinker", "amber" or "gromacs"',
)
...
if engname == "openmm":
    ...
    raise Exception(
        "OpenMM, SMIRNOFF/OpenMM, GROMACS, TINKER, and AMBER are supported at this time."
    )
...
def PrintEDA(EDA, N):
    ...
    for key, val in EDA.items():
        ...
        PrintDict[
            key
        ] = f"{val_avg: 12.4f} +- {val_err:10.4f} [ {val_avg/N: 9.4f} +- {val_err/N:7.4f} ]"
    printcool_dictionary(
...
def main():
    ...
    if engname in ["openmm", "smirnoff"]:
        ...
        if rpmd_beads > 0:
            raise RuntimeError("TINKER cannot handle RPMD.")
        if mts:
            logger.warn("Tinker not configured for multiple timestep integrator.")
    elif engname == "amber":
    ...
    Sep = printcool(
        f"Density: {Rho_avg: .4f} +- {Rho_err: .4f} kg/m^3\nAnalytic Derivative:"
    )
    ...
    Sep = printcool(
        f"Enthalpy of Vaporization: {Hvap_avg: .4f} +- {Hvap_err:.4f} kJ/mol\nAnalytic Derivative:"
    )
    ...
    Sep = printcool(
        f"Thermal expansion coefficient: {Alpha: .4e} +- {Alpha_err:.4e} K^-1\nAnalytic Derivative:"
    )
    ...
    if FDCheck:
        ...
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i-j: .4e}  {(i-j)/j: .4e}" for i, j in zip(GAlpha, GAlpha_fd)]
        FF.print_map(vals=absfrac)
    ...
    Sep = printcool(
        f"Isothermal compressibility:  {Kappa: .4e} +- {Kappa_err:.4e} bar^-1\nAnalytic Derivative:"
    )
    ...
    if FDCheck:
        ...
        Sep = printcool("Difference (Absolute, Fractional):")
        absfrac = [f"{i-j: .4e}  {(i-j)/j: .4e}" for i, j in zip(GKappa, GKappa_fd)]
        FF.print_map(vals=absfrac)
    ...
    GCp = GCp1 + GCp2 + GCp3
    Sep = printcool(
        f"Isobaric heat capacity:  {Cp: .4e} +- {Cp_err:.4e} cal mol-1 K-1\nAnalytic Derivative:"
    )
    ...
    Sep = printcool(
        f"Dielectric constant:           {Eps0: .4e} +- {Eps0_err:.4e}\nAnalytic Derivative:"
    )
