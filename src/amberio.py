        ...
        with open(fout, "w") as f:
            ...
            for name, block_dict, suffix in zip(names, block_dicts, suffixes):
                ...
                for key, val in block_dict.items():
                    print("%-20s ! %s" % (f"{key}={val},", cntrl_comm[key]), file=f)
                print("/", file=f)
                for line in suffix:
                    print("%s" % line, file=f)
        f.close()
...
class AMBER(Engine):
    ...
    def kineticE_cpptraj(self, leap=False, traj_fnm=None):
        ...
        print(
            f"Average temperature is {np.mean(temps):.2f}, kinetic energy {np.mean(kinetics):.2f}"
        )
...
"""
            ...
            self.callamber(
                f"nmode -O -i {self.name}-nr.in -c {self.name}.inpcrd -p {self.name}.prmtop -r {self.name}.rst -o {self.name}-nr.out"
            )
...
"""
        ...
        self.callamber(
            f"nmode -O -i {self.name}-nmode.in -c {self.name}.rst -p {self.name}.prmtop -v {self.name}-vecs.out -o {self.name}-vibs.out"
        )
