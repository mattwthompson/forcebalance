...
import forcebalance
from forcebalance.finite_difference import f1d5p, f1d7p, fdwrap
...
from forcebalance.output import DEBUG, CleanStreamHandler, getLogger
...
class Optimizer(forcebalance.BaseClass):
            ...
            if T.maxrd() == maxrd:
                ...
                if os.path.exists(os.path.join(T.absrd(), "mvals.txt")):
                    ...
                    if len(tmvals) > 0 and np.max(np.abs(tmvals - self.mvals0) > 1e-4):
                        warn_press_key(
                            f"mvals.txt in {T.absrd()} does not match loaded parameters.\nSave file : {tmvals}\\Parameters : {self.mvals0}\n"
                        )
                else:
    ...
    def MainOptimizer(self, b_BFGS=0):
        ...
        while 1:
            ...
            if ITERATION > self.iterinit and ndx < self.step_lowerbound:
                logger.info(
                    f"Step size is too small to continue ({ndx:.3e} < {self.step_lowerbound:.3e})\n"
                )
                break
            # ================================#
    ...
    def step(self, xk, data, trust):
        ...
        if Emin < self.eps:  # Mix in SD step if Hessian minimum eigenvalue is negative
            ...
            logger.info(
                f"Hessian has a small or negative eigenvalue ({Emin:.1e}), mixing in some steepest descent ({Adj:.1e}) to correct this.\n"
            )
        ...
        def trust_fun(L):
            ...
            logger.info(
                f"Finding trust radius: H{(L-1)**2:+.4f}*I, length {N:.4e} (target {trust:.4e})\n"
            )
        ...
        def h_fun(L):
            N = np.linalg.norm(solver(L)[0])
            logger.debug(
                f"\rL = {L:.4e}, Hessian diagonal addition = {(L-1)**2:.4e}: found length {N:.4e}, objective is {(N - trust)**2:.4e}\n"
            )
            return (N - self.h) ** 2
        ...
        def search_fun(L):
            ...
            logger.info(
                f"Hessian diagonal search: H{(L-1)**2:+.4f}*I, length {np.linalg.norm(dx):.4e}, result {Result: .4e}\n"
            )
            search_fun.micro += 1
            return Result
        ...
        if self.trust0 > 0:  # This is the trust region code.
            ...
            if dxnorm > trust:
                ...
                dxnorm = np.linalg.norm(dx)
                logger.info(
                    f"Trust-radius step found (length {dxnorm:.4e}), {(LOpt-1)**2: .4e} added to Hessian diagonal\n"
                )
            else:
        ...
        def xwrap(func, verbose=True):
            def my_func(mvals):
                ...
                if verbose:
                    ...
                    logger.info(f"X2= {color}{Answer:12.3e}\x1b[0m d(X2)= {dx:12.3e}\n")
                return Answer
