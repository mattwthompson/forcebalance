...
import forcebalance
from forcebalance.finite_difference import f1d2p, f12d3p, fdwrap_G, fdwrap_H, in_fd
...
class Target(with_metaclass(abc.ABCMeta, forcebalance.BaseClass)):
    ...
    def read(self, mvals, AGrad=False, AHess=False):
        ...
        if len(mvals) > 0 and (np.max(np.abs(mvals1 - mvals)) > 1e-3):
            warn_press_key(
                f"mvals from mvals.txt does not match up with get! (Are you reading data from a previous run?)\nmvals(call)={mvals} mvals(disk)={mvals1}"
            )
    ...
    def absrd(self, inum=None):
        ...
        if s[-1].startswith("iter_"):
            ...
            if not os.path.exists(os.path.join(abs_rd, self.name)):
                logger.error(
                    f"Target directory {self.name} does not exist in {self.rd}\n"
                )
                raise RuntimeError
            iterints = [
                int(d.replace("iter_", ""))
    ...
    def printcool_table(
        ...
        """
        tline = f"Target: {self.name}  Type: {self.__class__.__name__}  Objective = {self.objective:.5e}"
        nc = len(headings)
...
class RemoteTarget(Target):
    def __init__(self, options, tgt_opts, forcefield):
        ...
        tar.add(f"{self.root}/targets/{self.name}", arcname="targets/%s" % self.name)
        tar.close()
