...
class OpenMM(Engine):
    ...
    def setopts(self, platname="CUDA", precision="single", **kwargs):
        ...
        if self.platname not in valnames:
            warn_press_key(
                f"Platform {self.platname} does not exist (valid options are {valnames} (case-sensitive))"
            )
        ...
        if pbc:
            ...
            if self.AMOEBA:
                ...
                if nonbonded_cutoff > 0.05 * (float(int(minbox - 1))):
                    warn_press_key(
                        f"nonbonded_cutoff = {nonbonded_cutoff*10:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                    )
                if vdw_cutoff > 0.05 * (float(int(minbox - 1))):
                    warn_press_key(
                        f"vdw_cutoff = {vdw_cutoff*10:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                    )
