...
from pymbar import MBAR, bar as bar_function
import itertools
...
class Liquid(Target):
    ...
    def get_normal(self, mvals, AGrad=True, AHess=True):
        ...
        if len(BPoints) > 1:
            ...
            mbar = bar_function(U_kln, N_k, verbose=mbar_verbose, relative_tolerance=5.0e-8)
            W1 = mbar.weights()
            logger.info("Done\n")
        elif len(BPoints) == 1:
            W1 = np.ones((Shots,1))
        ...
        if len(mPoints) > 0:
            ...
            if len(mBPoints) > 1:
                ...
                if np.abs(np.std(mE)) > 1e-6 and mBSims > 1:
                    mmbar = bar_function(mU_kln, mN_k, verbose=False, relative_tolerance=5.0e-8, method='self-consistent-iteration')
                    mW1 = mmbar.weights()
            elif len(mBPoints) == 1:
