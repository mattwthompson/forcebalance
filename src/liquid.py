...
class Liquid(Target):
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if not os.path.exists("npt_result.p"):
            ...
            cmdstr = f"{self.nptpfx} python npt.py {self.engname} {temperature:.3f} {pressure:.3f}"
            if wq is None:
    ...
    def nvt_simulation(self, temperature):
        ...
        if not os.path.exists("nvt_result.p"):
            link_dir_contents(os.path.join(self.root, self.rundir), os.getcwd())
            cmdstr = f"{self.nptpfx} python nvt.py {self.engname} {temperature:.3f}"
            if wq is None:
                logger.info("Running condensed phase simulation locally.\n")
                logger.info(
    ...
    def indicate(self):
        ...
        def print_item(key, heading, physunit):
            if self.Xp[key] > 0:
                ...
                PrintDict[
                    heading
                ] = f"{self.Xp[key]: 10.5f} {self.Wp[key]: 8.3f} {self.Xp[key]*self.Wp[key]: 14.5e}"
    ...
    def objective_term(
        ...
        print_out = OrderedDict(
            [
                (
                    "    %8.2f %8.1f %3s" % PT,
                    f"{exp[PT]:9.3f}    {calc[PT]:9.3f} +- {err[PT]:<7.3f} {delt[PT]: 7.3f} {Weights[PT]: 9.5f} {Objs[PT]: 9.5f}",
                )
                for PT in calc
            ]
        )
    ...
    def read(self, mvals, AGrad=True, AHess=True):
        ...
        if len(mvals) > 0 and (np.max(np.abs(mvals1 - mvals)) > 1e-3):
            warn_press_key(
                f"mvals from forcebalance.p does not match up with internal values! (Are you reading data from a previous run?)\nmvals(call)={mvals} mvals(disk)={mvals1}"
            )
