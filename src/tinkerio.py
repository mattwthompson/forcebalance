...
from forcebalance.molecule import BuildLatticeFromLengthsAngles, Molecule
...
pdict = {
    "VDW": {
        ...
        3: "T",
        4: "D",
    },  # Van der Waals distance, well depth, distance from bonded neighbor?
    "BOND": {
        "Atom": [1, 2],
        3: "K",
        4: "B",
    },  # Bond force constant and equilibrium distance (Angstrom)
    "ANGLE": {
        "Atom": [1, 2, 3],
        4: "K",
        5: "B",
    ...
    "STRBND": {
        ...
        4: "K1",
        5: "K2",
    },  # Two stretch-bend force constants (usually same)
    "OPBEND": {"Atom": [1, 2, 3, 4], 5: "K"},  # Out-of-plane bending force constant
    "UREYBRAD": {
        "Atom": [1, 2, 3],
        4: "K",
        5: "B",
    },  # Urey-Bradley force constant and equilibrium distance (Angstrom)
    "TORSION": (
        {"Atom": [1, 2, 3, 4], 5: "1K", 6: "1B", 8: "2K", 9: "2B", 11: "3K", 12: "3B"}
    ),  # Torsional force constants and equilibrium phi-angles
...
def write_key(fout, options, fin=None, defaults={}, verbose=False, prmfnm=None, chk=[]):
    ...
    options = OrderedDict(
        [
            (key.lower(), str(val) if val is not None else None)
        ...
        for line0 in open(fin).readlines():
            ...
            if key == "parameters":
                ...
                if prmfnm is None or val0 in prms or val0[:-4] in prms:
                    ...
                    logger.info(line + "\n")
                    warn_press_key(
                        f"The above line was found in {line}, but we expected something like 'parameters {prmfnm}'; replacing."
                    )
            ...
            if key in options:
                ...
                if key in clashes and val != val0:
                    logger.error(
                        f"write_key tried to set {key} = {val} but its original value was {key} = {val0}\n"
                    )
                    raise RuntimeError
...
class TINKER(Engine):
            ...
            if "Version" in line:
                ...
                try:
                    if vn < vn_need:
                        if self.warn_vn:
                            warn_press_key(
                                "ForceBalance requires TINKER %.1f - unexpected behavior with older versions!"
                                % vn_need
                            )
                        self.warn_vn = True
                except:
    ...
    def prepare(self, pbc=False, **kwargs):
        ...
        if pbc:
            ...
            if rpme > 0.5 * (float(int(minbox - 1))):
                warn_press_key(
                    f"nonbonded_cutoff = {rpme:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                )
            ...
            if rvdw > 0.5 * (float(int(minbox - 1))):
                warn_press_key(
                    f"vdw_cutoff = {rvdw:.1f} should be smaller than half the box size = {minbox:.1f} Angstrom"
                )
        ...
        write_key(
            "%s.key" % self.name,
            tk_opts,
            os.path.join(self.srcdir, self.key) if self.key else None,
            tk_defs,
            verbose=False,
            prmfnm=prmfnm,
        )
        ...
        for line in o:
            ...
            if (
                "Atom Type Definition Parameters" in line
        ...
        if len(list(G.nodes())) > 0:
            ...
            tmols = [
                gs[i] for i in np.argsort(np.array([min(list(g.nodes())) for g in gs]))
            ]
            mnodes = [list(m.nodes()) for m in tmols]
            self.AtomLists["MoleculeNumber"] = [
                [i + 1 in m for m in mnodes].index(1) for i in range(self.mol.na)
            ]
        else:
            grouped = [i.L() for i in self.mol.molecules]
            self.AtomLists["MoleculeNumber"] = [
                [i in g for g in grouped].index(1) for i in range(self.mol.na)
            ]
        if prmtmp:
    ...
    def optimize(self, shot, method="newton", align=True, crit=1e-4):
        ...
        o = self.calltinker(f"{optprog} {self.name}.xyz {crit:f}")
    ...
    def evaluate_(self, xyzin, force=False, dipole=False):
        ...
        if dipole or (not force):
            oanl = self.calltinker(
                f"analyze {xyzin} -k {self.name}", stdin="G,E,M", print_to_screen=False
            )
        ...
        if force:
            ...
            F = []
            Fi = []
            o = self.calltinker(f"testgrad {xyzin} -k {self.name} y n n")
            i = 0
            ReadFrc = 0
            for line in o:
                s = line.split()
    ...
    def normal_modes(self, shot=0, optimize=True):
        ...
        for line in o:
            ...
            if "Vibrational Normal Mode" in line:
                ...
                readev
                and len(s) == 4
                and all([isint(s[0]), isfloat(s[1]), isfloat(s[2]), isfloat(s[3])])
            ):
    ...
    def energy_rmsd(self, shot=0, optimize=True):
        ...
        if optimize and abs(E - E_) > 0.1:
            warn_press_key(
                f"Energy from optimize and analyze aren't the same ({E:.3f} vs. {E_:.3f})"
            )
        return E, rmsd
    ...
    def molecular_dynamics(
        ...
        **kwargs,
        ...
        if minimize:
            if verbose:
                logger.info("Minimizing the energy...")
            self.optimize(0, method="bfgs", crit=1)
            os.system(f"mv {self.name}.xyz_2 {self.name}.xyz")
            if verbose:
                logger.info("Done\n")
        ...
        # Gather information.
        os.system(f"mv {self.name}.arc {self.name}-md.arc")
        ...
        if verbose:
            logger.info("Post-processing to get the dipole moments\n")
        oanl = self.calltinker(
            "analyze %s-md.arc" % self.name, stdin="G,E,M", print_to_screen=False
        )
                ...
                np.array(
                    [
                        ...
                        if (
                            ...
                            and all([isfloat(i) for i in line.split()[:6]])
                        )
                    ]
                )
                / 1000
...
class Liquid_TINKER(Liquid):
    ...
    def __init__(self, options, tgt_opts, forcefield):
        ...
        self.set_option(
            tgt_opts, "liquid_coords", default="liquid.xyz", forceprint=True
        )
        ...
        self.gas_engine_args = {
            "tinker_key": "%s.key" % os.path.splitext(self.gas_coords)[0]
        }
    ...
    def npt_simulation(self, temperature, pressure, simnum):
        ...
        if self.goodstep and (temperature, pressure) in self.DynDict_New:
            self.DynDict[(temperature, pressure)] = self.DynDict_New[
        ...
        if (temperature, pressure) in self.DynDict:
            ...
            logger.info(f"Copying .dyn file: {dynsrc} to {dyndest}\n")
            shutil.copy2(dynsrc, dyndest)
            self.nptfiles.append(dyndest)
        self.DynDict_New[(temperature, pressure)] = os.path.join(
            os.getcwd(), "liquid.dyn"
        )
