...
from forcebalance import optimizer, parser
...
GenOptionDoc = {
    "gmxpath": {
        "scope": "Targets that use GROMACS",
        "required": True,
    ...
    "penalty_type": {
        "scope": "All force field optimizations",
        "required": False,
    ...
    "scan_vals": {
        "scope": "scan_mvals and scan_pvals job types",
        "required": False,
    ...
    "maxstep": {
        "scope": "All iterative optimization jobs",
        "required": False,
        "recommend": "At least 100 optimization steps are recommended.",
    },
    "readchk": {
        "scope": "Main optimizer",
        "required": False,
    ...
    "writechk_step": {
        "scope": "Main optimizer when 'writechk' is turned on",
        "required": False,
    ...
    "forcefield": {"scope": "All force field optimizations", "required": True},
    "backup": {"scope": "All force field optimizations", "required": False},
    "trust0": {
        "scope": "Main optimizer",
        "required": False,
    ...
    "finite_difference_h": {
        "scope": "fdcheck_G or fdcheck_H job types, or whenever the objective function is evaluated using finite difference",
        "required": False,
    ...
    "amoeba_polarization": {
        "scope": "Optimizations of the AMOEBA polarizable force field",
        "required": False,
...
""",
    ...
    "asynchronous": {
        "scope": "Targets that use Work Queue",
        "required": False,
    ...
    "constrain_charge": {
        "scope": "Force fields with point charges",
        "required": False,
...
TgtOptionDoc = {
    ...
    "type": {
        ...
        "long": """This is the type of target that you are running.  The current accepted values for the target type
                             are given in the beginning of the objective.py file: %s."""
        % ", ".join([i for i in Implemented_Targets]),
    ...
    "shots": {
        "scope": "Force and energy matching",
        "required": False,
...
def create_index(optiondoc, opt_types):
    ...
    for i in ["strings", "allcaps", "lists", "ints", "bools", "floats", "sections"]:
        ...
        for j in opt_types[i]:
            val = opt_types[i][j][0]
            # print j
            Glossary[j] = [f"@li <b> {j.upper()} </b> ({vartype.capitalize()})"]
            Glossary[j].append(
...
def main():
    ...
    Answer = [
        r"""\section gen_option_index Option index: General options
...
"""
    ...
    Answer.append(
        r"""\section tgt_option_index Option index: Target options
