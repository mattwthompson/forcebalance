...
def build(interactive=False, upstream=False):
    ...
    try:
        ...
        if subprocess.call(["doxygen", "doxygen.cfg"]):
            raise OSError("Doxygen returned nonzero value while working on doxygen.cfg")
        ...
        if subprocess.call(["doxygen", "api.cfg"]):
            raise OSError("Doxygen returned nonzero value while working on api.cfg")
        ...
        display(
            "cp Images/ForceBalance.pdf latex/ && cp Images/ForceBalance.pdf latex/api/"
        )
        ...
        if subprocess.call(["make"]):
            raise OSError("make returned nonzero value while compiling latex/")
    ...
    if upstream:
        ...
        for fnm in ["latex", "html"]:
            display(f"mv {fnm} {fnm}_")
            os.system(f"mv {fnm} {fnm}_")
        ...
        display(
            'git commit -a -m "Automatic documentation generation at {} on {}"'.format(
                gethostname(), datetime.now().strftime("%m-%d-%Y %H:%M")
            )
        )
        if os.system(
            'git commit -a -m "Automatic documentation generation at {} on {}"'.format(
        ...
        try:
            ...
            if os.system("git add html latex"):
                raise OSError("Error trying to stage files for commit")
            ...
            if os.system("git push"):
                raise OSError(
                    "While trying to push changes upstream 'git push' gave a nonzero return code"
                )
...
"""
    ...
    for fnm in os.listdir("./html/api/"):
        if re.match(r".*\.html$", fnm):
            fnm = "./html/api/" + fnm
            newfile = []
            for line in open(fnm):
                if re.match(r'.*a href="index\.html"', line):
                    newfile.append(
                        '      <li><a href="../index.html"><span>Main Page</span></a></li>\n'
                    )
                    newfile.append("      <li ")
                    if re.match(r".*roadmap\.html$", fnm):
                        newfile.append('class="current"')
                    newfile.append(
                        '><a href="roadmap.html"><span>Project Roadmap</span></a></li>\n'
                    )
                else:
                    newfile.append(line)
...
def find_forcebalance():
    ...
    try:
        ...
        print(
            "Try running forcebalance/setup.py or you can always set the INPUT directory"
        )
...
def find_doxypy():
    ...
    try:
        ...
        if subprocess.call(["doxypy", "makedocumentation.py"], stdout=open(os.devnull)):
            raise OSError()
        doxypy_path = "doxypy"
    except OSError:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-i",
        action="store_true",
        help="run in interactive mode, pausing before each command",
    )
    parser.add_argument(
        "--clean",
        "-k",
        action="store_true",
        help="remove temporary files after script is complete",
    )
    parser.add_argument(
        "--configure",
        "-c",
        action="store_true",
        help="generate doxygen configuration files from templates",
    )
    parser.add_argument(
        "--upstream",
        "-u",
        action="store_true",
        help="push updated documentation to upstream github repository",
    )
    ...
    if args.configure:
        ...
        print(
            "Couldn't find required doxygen config files ('./doxygen.cfg' and './api.cfg').\nRun with --configure option to generate automatically"
        )
        sys.exit(1)
