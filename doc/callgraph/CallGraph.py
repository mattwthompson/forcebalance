...
def main():
                ...
                if not indeclar and t[0] not in knowntypeslist:
                    try:
                        ...
                        for _, token, _, _, _ in g:
                            ...
                            if len(matches) > 1:
                                print(
                                    "Multiple matches detected for",
                                    token,
                                    file=sys.stderr,
                                )
                                print(
                                    "!!! Graph may contain errors !!!", file=sys.stderr
                                )
                                for obj in matches:
    ...
    for module in modulelist:
        thisid = [
            ...
            if obj.name == module and obj.dtype == "file"
        ][0]
        print(
            "    ",
            thisid,
            '[ label="'
            + module
            + '", color = grey, style="rounded,filled", fillcolor = yellow]',
        )
    print("}")
