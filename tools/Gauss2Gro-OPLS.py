...
from numpy import arccos, array, dot, mean, pi, sort, std, zeros
    ...
    for Type in TypeList:
        ...
        if not ManualNow:
            ...
            if ReadBaseVals:
                ...
                print(
                    f"Automatic parameterization of {Type} {InteractionName} using base geometry, std deviation is {std(BaseVals):.6f}"
                )
            ...
            if ReadBaseVals:
                print(
                    f"Standard deviation of all equilibrium {InteractionName} of this type is {std(BaseVals):.6f}"
                )
            line = input(
                "\nFor a '{}' interaction involving {} (OPLS types {}), \nselect line number from above, type your own parameters, \nor hit Enter to accept default. (# of \x1b[91m^\x1b[0m symbols indicate score) --> ".format(
                    InteractionName,
                    "-".join(Type.split(".")),
...
for i in sorted(list(set(T))):
    ...
    OPLSChoices = list(
        os.popen(
            f"awk '$2 ~ /{Element}/' {oplsnb} | awk '(($4 - {PeriodicTable[Element]:.3f}) < 0.1 && ($4 - {PeriodicTable[Element]:.3f}) > -0.1)'"
        ).readlines()
    )
    ChoicesByName = {}
    for line in OPLSChoices:
