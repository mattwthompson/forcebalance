...
class SCF_Simulation:
    def __init__(self, root, name):
        self.name = name
        self.root = os.path.join(os.path.abspath(root), name)
        if not os.path.exists(self.root):
            raise Exception(
                "The root directory for simulation %s (%s) doesn't exist"
                % (self.name, self.root)
            )
    ...
    def SetupGeneration(self):
        ...
        if not os.path.exists("settings"):
            raise Exception(
                "The settings directory doesn't exist for simulation %s - please create it and add the necessary files"
                % self.name
            )
        if options["gmxpath"] == None or options["gmxsuffix"] == None:
            warn_press_key(
                "Please set the options gmxpath and gmxsuffix in the input file!"
            )
        if not os.path.exists(
            os.path.join(options["gmxpath"], "mdrun" + options["gmxsuffix"])
        ):
            warn_press_key(
                "The mdrun executable pointed to by %s doesn't exist! (Check gmxpath and gmxsuffix)"
                % os.path.join(options["gmxpath"], "mdrun" + options["gmxsuffix"])
            )
    ...
    def BuildMSM(self):
        ...
        LinkFile(
            os.path.join(self.root, GenDir, "dynamics", "traj.xtc"),
            os.path.join(os.getcwd(), "traj.xtc"),
        )
        Leave("RawData/RUN00")
        _exec("ConvertDataToHDF.py -s conf.pdb -i RawData")


class ForceBalance_SCF:
    def __init__(self):
        self.Mao = 0
        self.root = os.getcwd()
