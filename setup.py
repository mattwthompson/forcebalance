...
def buildKeywordDictionary(args):
    ...
    setupKeywords["scripts"] = (
        glob.glob("bin/*.py")
        + glob.glob("bin/*.sh")
        + glob.glob("bin/*.bash")
        + glob.glob("bin/ForceBalance")
        + glob.glob("bin/TidyOutput")
    )
    ...
    if args.test:
        ...
        test_data = (
            glob.glob("files/*.*")
            + glob.glob("files/forcefield/*.*")
            + glob.glob("files/targets/*/*.*")
            + glob.glob("files/*.*")
            + ["files/work_queue_worker"]
        )
        os.chdir("..")
...
def doClean():
    ...
    subprocess.call(
        "rm -f %s/../forcebalance-*.egg-info" % forcebalance_dir, shell=True
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "--dirty",
        action="store_true",
        help="don't remove previously installed forcebalance installation first",
    )
    parser.add_argument(
        "-t", "--test", action="store_true", help="install forcebalance test suite"
    )
    parser.add_argument(
        "-g", "--gui", action="store_true", help="install forcebalance gui module"
    )
    ...
    try:
        ...
        print(
            "Error importing bz2, which is important for distributed calculations and remote targets"
        )
        print("Please either (1) make sure Python is built/installed with bz2 support")
        print(
            "or (2) proceed with inefficient reading/writing of files; remote targets won't work."
        )
